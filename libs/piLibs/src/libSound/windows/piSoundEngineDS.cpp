...
int piSoundEngineDS::AddSound(const wchar_t *filename)
{
    ...
    me->mWasPlaying = false;
    me->mWasLooping = false;
    me->mID = id;
...
void piSoundEngineDS::PauseAllSounds(void)
{
    const int num = mSounds.GetLength();
	for( int i = 0; i < num; i++ )
	{
		iSound *me = (iSound*)mSounds.GetAddress(i);
		DWORD status;
...
void piSoundEngineDS::ResumeAllSounds(void)
{
    const int num = mSounds.GetLength();
	for( int i = 0; i < num; i++ )
	{
		...
		if( me->mWasPlaying )
		{
			me->mBuffer->Play(0, 0, (me->mWasLooping) ? DSBPLAY_LOOPING : 0);
